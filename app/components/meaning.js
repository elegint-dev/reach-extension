...
//     basis_ref: "https://…",       // reference only: the document cited (URL or repo path)
//     data_model: "Endpoint.Processes",  // reference from Splunk's CIM docs
...
const SOURCES = {
  ...
  reference: { trust: "asserted", line: "From the cited documentation. Not validated against the corpus." },
...
export function meaning(props = {}) {
  const { description, hunting_notes, data_format, source, confidence, evidence, statement, absent, basis_ref, data_model } = props;
  ...
  return h(
    ...
    h(
      ...
      basis_ref ? h("span", { class: "r-meaning__ref" }, " ", citation(basis_ref, data_model)) : null,
    ),
...
// A URL becomes a link; a repo path stays text. Data model, when known, leads.
function citation(ref, dataModel) {
  const label = dataModel ? `CIM ${dataModel}` : ref;
  return /^https?:\/\//.test(ref)
    ? h("a", { href: ref, rel: "noreferrer", target: "_blank", class: "r-idlink" }, label)
    : h("code", null, ref);
}
