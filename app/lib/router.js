...
//                        onChange(state, { replaced }) where replaced is true for
//                        the hashchange replace() dispatches on the same history entry
// navigate(route, params)
// replace(route, params) → the same history entry under a new hash
...
export const ROUTES = Object.freeze([
  ...
  { route: 'sourcetype', pattern: '/st', param: 'name' },
  { route: 'catalogue', pattern: '/catalogue', param: null },
  { route: 'share', pattern: '/share', param: null },
  { route: 'discover', pattern: '/discover', param: null },
  { route: 'packs', pattern: '/packs', param: null },
  { route: 'coverage', pattern: '/coverage', param: null },
  { route: 'notebook', pattern: '/notebook', param: null },
  { route: 'runbook', pattern: '/runbook', param: 'key' },
]);
...
// replace() dispatches its own hashchange so the page re-renders; the flag
// tells start()'s listener that no history entry was added.
let replacing = false;
...
// Calls onChange(state, meta) now and on every hashchange. Returns a stop function.
export function start(onChange, target = win()) {
  const fire = () => onChange(parse(target && target.location ? target.location.hash : ''), { replaced: replacing });
