...
//              seeded_from: { source, id, url, label, ref, author } | null,
...
export function check(raw) {
  ...
  return {
    ...
    seeded_from: sf ? { source: text(sf.source, 40) || null, id: text(sf.id, 120) || null, url: /^https?:\/\//.test(str(sf.url, 500)) ? text(sf.url, 500) : null, label: text(sf.label, 100) || null, ref: text(sf.ref, 100) || null, author: text(sf.author, 200) || null } : null,
...
export function fromSeed(seed) {
  ...
  return check({
    ...
    title: seed.title,
    rule: seed.rule,
    seeded_from: seed.seeded_from ? { source: seed.seeded_from.source, id: seed.seeded_from.id, url: seed.seeded_from.url, label: seed.seeded_from.label, ref: seed.seeded_from.ref, author: seed.seeded_from.author || null } : null,
