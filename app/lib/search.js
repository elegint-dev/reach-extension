...
//   classify()'s cve case mirrors app/lib/shapes.js detectCve(): without it a
//   pasted CVE id falls into the default name branch and the value page's
//   enrich band (CISA KEV) never draws (app/lib/enrich/kev.js is unaffected;
//   its own classify() already reads detectCve() directly).
//
...
export const KINDS = Object.freeze([
  'sha256', 'sha1', 'md5_or_aid', 'cve', 'ipv4', 'ipv6', 'domain', 'os_pid', 'falcon_pid', 'path', 'name', 'empty',
]);
...
const CARRIERS = Object.freeze({
  ...
  image: ['ImageFileName', 'CommandLine'],
  file: ['TargetFileName'],
  registry: ['RegObjectName', 'RegistryPath'],
  cve: [],
  name: [],
});

// A path is read by its shape: a registry key, an executable image or loaded
// module, a written or accessed file. Executables and extensionless Unix paths
// get both the image and the file reading; the value page shows one band each.
const REGISTRY_RE = /^(?:\\REGISTRY\\|HKEY_[A-Z_]+\\|HK(?:LM|CU|CR|U|CC)\\)/i;
const MODULE_EXTENSIONS = new Set(['exe', 'dll', 'sys', 'scr', 'com', 'ocx', 'cpl', 'drv', 'efi', 'so', 'dylib', 'ko', 'kext']);

export function pathReadings(value) {
  if (REGISTRY_RE.test(value)) return [cand('registry', 'registry path', CARRIERS.registry)];
  const leaf = value.split(/[\\/]/).pop() || '';
  const dot = leaf.lastIndexOf('.');
  const ext = dot > 0 ? leaf.slice(dot + 1).toLowerCase() : '';
  const image = MODULE_EXTENSIONS.has(ext) || (ext === '' && value.startsWith('/'));
  const file = cand('file', 'written or accessed file', CARRIERS.file);
  return image ? [cand('image', 'image name / loaded module', CARRIERS.image), file] : [file];
}
...
export function classify(input, index) {
  ...
  if (cve) return { kind: 'cve', value, candidates: [cand('cve', 'CVE identifier', CARRIERS.cve)] };
  ...
  if (value.includes('/') || value.includes('\\')) {
    return { kind: 'path', value, candidates: pathReadings(value) };
  }
